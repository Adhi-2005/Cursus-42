...
/*   Updated: 2024/03/30 03:26:23 by adshafee         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// Function to validate the new position
int validate_position(t_array *game, size_t new_y, size_t new_x) {
    // Check if the movement is within the boundaries of the game map
    if (game == NULL || game->map == NULL) {
        ft_printf("Error: Game map is not initialized\n");
        return 0;
    }

    // Check if the new position is within the boundaries of the map
    if (new_y >= game->breadth || new_x >= game->length) {
        ft_printf("Error: Attempted movement outside of map boundaries\n");
        return 0;
    }

    char new_pos = game->map[new_y][new_x];

    // Handle different cases based on the new position
    if (new_pos == '1') {
        ft_printf("You can't move into a wall\n");
        return 0;
    } else if (new_pos == 'E' && game->num_of_collectibles > 0) {
        ft_printf("You need to collect all the collectibles before you exit\n");
        return 0;
    }
...
void handle_movement(t_array *game, size_t new_y, size_t new_x) {
    // Update player position and collectibles count
    ...
    if (game->map[new_y][new_x] == 'E' && game->num_of_collectibles == 0) {
        ft_printf("YOU WON !!!\n");
        finish(game);
    }
}
...
void ft_move_player(t_array *game, size_t a, size_t b) {
    ...
    if (!validate_position(game, new_y, new_x)) {
        return;
    }
